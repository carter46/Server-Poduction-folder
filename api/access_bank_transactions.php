...
switch ($method) {
    case 'GET':
        ...
        try {
            $stmt = $pdo->query("SELECT COUNT(*) as total FROM access_bank_transactions");
            $total = $stmt->fetch()['total'];

            $stmt = $pdo->prepare("SELECT * FROM access_bank_transactions ORDER BY transaction_date DESC LIMIT ? OFFSET ?");
    ...
    case 'POST':
        ...
        try {
            ...
            $transactionId = txInsertBankTransaction($pdo, 'access_bank_transactions', [
            ...
            try {
                ...
                if ($transaction) {
                    mobileNotifyBeneficiaryCredit($pdo, $transaction, 'access_bank_transactions', 'access');
                }
            } catch (Exception $e) {
            }
