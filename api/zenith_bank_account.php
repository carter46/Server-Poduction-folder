<?php
/**
 * Zenith Bank Account Settings API
 * Handles account name, account number, and balance management for Zenith Bank
 */
...
switch ($method) {
    case 'GET':
        ...
        try {
            $stmt = $pdo->query("SELECT account_name, account_number, balance FROM zenith_bank_account_settings ORDER BY id DESC LIMIT 1");
            $account = $stmt->fetch();

            if (!$account) {
                // Initialize with default values if no account exists
                $stmt = $pdo->prepare("INSERT INTO zenith_bank_account_settings (account_name, account_number, balance) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            ...
            $sql = "UPDATE zenith_bank_account_settings SET " . implode(", ", $updates) . " WHERE id = ?";
            $params[] = $accountId;
