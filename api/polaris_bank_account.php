...
switch ($method) {
    case 'GET':
        try {
            $stmt = $pdo->query("SELECT account_name, account_number, balance FROM polaris_bank_account_settings ORDER BY id DESC LIMIT 1");
            $account = $stmt->fetch();

            if (!$account) {
                $stmt = $pdo->prepare("INSERT INTO polaris_bank_account_settings (account_name, account_number, balance) VALUES (?, ?, ?)");
                $stmt->execute(['AUTOGRAPH CONSTRUCTION LIMITED', '1762090307', 4192401.00]);

                $account = [
                    'account_name' => 'AUTOGRAPH CONSTRUCTION LIMITED',
                    'account_number' => '1762090307',
                    'balance' => 4192401.00
                ];
            }
