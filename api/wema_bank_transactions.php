<?php
/**
 * Wema Bank Transactions API
 * Handles transaction creation, retrieval, status update, and deletion for Wema Bank
 */
...
switch ($method) {
    case 'GET':
        ...
        try {
            $stmt = $pdo->query("SELECT COUNT(*) as total FROM wema_bank_transactions");
            $total = $stmt->fetch()['total'];

            $stmt = $pdo->prepare("SELECT * FROM wema_bank_transactions ORDER BY transaction_date DESC LIMIT ? OFFSET ?");
    ...
    case 'POST':
        ...
        try {
            ...
            $transactionId = txInsertBankTransaction($pdo, 'wema_bank_transactions', [
            ...
            $stmt = $pdo->query("SELECT id FROM wema_bank_account_settings ORDER BY id DESC LIMIT 1");
            $accountRow = $stmt->fetch();
            $accountId = $accountRow['id'];

            $stmt = $pdo->prepare("UPDATE wema_bank_account_settings SET balance = balance - ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([floatval($input['amount']), $accountId]);

            $pdo->commit();

            $stmt = $pdo->prepare("SELECT * FROM wema_bank_transactions WHERE id = ?");
            ...
            try {
                ...
                if ($transaction) {
                    mobileNotifyBeneficiaryCredit($pdo, $transaction, 'wema_bank_transactions', 'wema');
                }
            } catch (Exception $e) {
            }
    ...
    case 'PUT':
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, amount, status FROM wema_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $pdo->prepare("UPDATE wema_bank_transactions SET status = ? WHERE id = ?");
    ...
    case 'DELETE':
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT amount, status FROM wema_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
            $transaction = $stmt->fetch();
            ...
            $stmt = $pdo->prepare("DELETE FROM wema_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
