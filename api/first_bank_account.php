<?php
/**
 * First Bank Account Settings API
 * Handles account name, account number, and balance management for First Bank
 */
...
switch ($method) {
    case 'GET':
        ...
        try {
            $stmt = $pdo->query("SELECT account_name, account_number, balance FROM first_bank_account_settings ORDER BY id DESC LIMIT 1");
            $account = $stmt->fetch();
            
            if (!$account) {
                // Initialize with default values if no account exists
                $stmt = $pdo->prepare("INSERT INTO first_bank_account_settings (account_name, account_number, balance) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            ...
            $sql = "UPDATE first_bank_account_settings SET " . implode(", ", $updates) . " WHERE id = ?";
            $params[] = $accountId;
