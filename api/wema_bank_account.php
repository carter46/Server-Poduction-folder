<?php
/**
 * Wema Bank Account Settings API
 * Handles account name, account number, and balance management for Wema Bank
 */
...
switch ($method) {
    case 'GET':
        try {
            $stmt = $pdo->query("SELECT account_name, account_number, balance FROM wema_bank_account_settings ORDER BY id DESC LIMIT 1");
            $account = $stmt->fetch();

            if (!$account) {
                $stmt = $pdo->prepare("INSERT INTO wema_bank_account_settings (account_name, account_number, balance) VALUES (?, ?, ?)");
                $stmt->execute(['AUTOGRAPH CONSTRUCTION LIMITED', '1022090307', 4192401.00]);
    ...
    case 'PUT':
        ...
        try {
            ...
            $sql = "UPDATE wema_bank_account_settings SET " . implode(", ", $updates) . " WHERE id = ?";
            $params[] = $accountId;
