<?php
/**
 * Stanbic IBTC Bank Transactions API
 */
...
switch ($method) {
    case 'GET':
        ...
        try {
            $stmt = $pdo->query("SELECT COUNT(*) as total FROM stanbic_bank_transactions");
            $total = $stmt->fetch()['total'];

            $stmt = $pdo->prepare("SELECT * FROM stanbic_bank_transactions ORDER BY transaction_date DESC LIMIT ? OFFSET ?");
    ...
    case 'POST':
        ...
        try {
            ...
            $transactionId = txInsertBankTransaction($pdo, 'stanbic_bank_transactions', [
            ...
            $stmt = $pdo->prepare("UPDATE stanbic_bank_account_settings SET balance = balance - ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([floatval($input['amount']), $accountId]);

            $pdo->commit();

            $stmt = $pdo->prepare("SELECT * FROM stanbic_bank_transactions WHERE id = ?");
            ...
            try {
                ...
                if ($transaction) {
                    mobileNotifyBeneficiaryCredit($pdo, $transaction, 'stanbic_bank_transactions', 'stanbic');
                }
            } catch (Exception $e) {
            }
    ...
    case 'PUT':
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, amount, status FROM stanbic_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $pdo->prepare("UPDATE stanbic_bank_transactions SET status = ? WHERE id = ?");
    ...
    case 'DELETE':
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT amount, status FROM stanbic_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
            $transaction = $stmt->fetch();
            ...
            $stmt = $pdo->prepare("DELETE FROM stanbic_bank_transactions WHERE id = ?");
            $stmt->execute([$id]);
