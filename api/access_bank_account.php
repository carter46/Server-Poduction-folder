<?php
/**
 * Access Bank Account Settings API
 * Handles account name, account number, and balance management for Access Bank
 */
...
switch ($method) {
    case 'GET':
        try {
            $stmt = $pdo->query("SELECT account_name, account_number, balance FROM access_bank_account_settings ORDER BY id DESC LIMIT 1");
            $account = $stmt->fetch();

            if (!$account) {
                $stmt = $pdo->prepare("INSERT INTO access_bank_account_settings (account_name, account_number, balance) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            ...
            $sql = "UPDATE access_bank_account_settings SET " . implode(", ", $updates) . " WHERE id = ?";
            $params[] = $accountId;
